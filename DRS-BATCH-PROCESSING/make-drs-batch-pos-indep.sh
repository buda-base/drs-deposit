...
if [ ! -f $worksList ]; then
	echo "${0}: worksList \'${1}\' does not exist or is not a directory"
	exit 2
fi
...
if [ ! -d $projectMaster ]; then
	echo "${0}: projectMaster \'${2}\' does not exist or is not a directory"
	exit 2
fi
...
if [ ! -d $archiveDir ]; then
	echo "${0}: archiveDir \'${4}\' does not exist or is not a directory"
	exit 2
fi
...
echo BB: $bbDir $bb
if [ ! -d $bbDir ]; then
	echo "${0}: BatchBuilder directory  \'${5}\' does not exist or is not a directory"
	exit 2
fi
if [ ! -f $bbDir/$bb ]; then
...
# jsk: Target project is absolute
targetProjectDir=$3
...
if [ -e $targetProjectDir ]; then
	echo "targetProjectDir ${targetProjectDir} already exists. Remove it or use a different name"
	exit 2
fi

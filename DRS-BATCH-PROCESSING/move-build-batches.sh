...
export SRC=$PR/batchBuildsFromBodhi
...
cat  $DICT | grep batchW |  awk -F, '{ split($12,dt,"T"); print($10, dt[1])}' |  sort -u  | parallel --colsep ' ' 'if [[ -d $SRC/{1} ]] ; then mkdir -p $TARGET/{2} ; mv -v $SRC/{1} $TARGET/{2} ; else echo {1} not found ; fi'

...
import csv
import pathlib
from drs_db_apps.Writers.listwriter import ListWriter
...
class CSVWriter(ListWriter):
    ...
    def PutResultSets(self, results: list, fieldNames: list) -> None:
        ...
        with self.osPath.open("w", newline='') as fw:
            # Create the CSV writer. NOTE: multiple headers are written to the
            # one output file
            csv_writer = csv.DictWriter(fw, fieldNames, lineterminator='\n')

...
from BdrcDbLib.DbAppParser import DbArgNamespace, str2date, DbAppParser, mustExistDirectory
    ...
    def do_update(self) -> None:
        ...
        try:
            ...
            if self._options.delete:
                ...
                u_cursor.execute(
                    f'insert ignore BuildPaths ( `BuildPath`,`build_dir`) values ("{build_path}","{build_dir}") ;')
                conn.commit()
                ...
                for volDir in volumes_for_batch(build_path):
                    ...
                    print(
                        f"{volDir}, {build_path}, {self._options.buildDate}, {self._options.result}, {vol_files}, ' \
                        '{vol_size}")

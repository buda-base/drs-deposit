...
class ErrorScanner:
    ...
    def process(self, aLine):
        ...
        # the first two fields are just the result of the scan through the
        # bbconsole.txts
        file = lineFrags[0]
        ...
        # HACK: This removes colons from the raw text. Search strings have
        ...
        for anError in iter(self.errorSources):
            ...
            if anError[1] in bbConsoleText:
                ...
                # self.errorResults.append(errorBead.ErrorBead(bbConsoleHome, \
                # bbConsoleLine, anError, bbConsoleText))
                # jsk 1/18: lets have the full path - we'll need it
                self.errorResults.append(errorBead.ErrorBead(file,
                                                             bbConsoleLine,
                                                             anError,
                                                             bbConsoleText))
            # ------------ else:
...
#         """Scans the file for error patterns and returns the error
#         beads of errors we care about"""
#         with fileinput.input(files=(inputFilePath)) as f:
...
#         """searches for a set of texts in the line, returns a new error
# 			bead"""
#         keepGoing = True
#         while keepGoing:
...
#                 continue
#
#             # the first two fields are just the result of the scan through \
#              the bbconsole.txts
#             file=lineFrags[0]
#             bbConsoleLine=lineFrags[1]
#             # We always know the file is bb-console.txt
...
#             # TODO Use regexps to speed up
#             for anError in iter(self.errorSources):
#                  print(f"+ {self.lineCount}__{bbConsoleLine}_|\
# 						_{bbConsoleText}")
#                 if anError[1] in bbConsoleText:
#                     print(f"found in {bbConsoleLine} {bbConsoleText}")

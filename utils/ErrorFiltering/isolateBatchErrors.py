...
from ErrorFiltering.errorparsers import TikaParse, WorkThenVolume, EXIFWork

from ErrorFiltering.errorLabels import *
from ErrorFiltering.errorprinters import *

knownErrors = []
"""List of possible errors. Key is a mnemonic constant, value is the text in
the input which indicates the error. See loadErrors"""
...
def loadErrors():
    ...
    knownErrors.append(BBErrorKey(FAIL_MODS_ID, FAIL_MODS_CALL,
                                  lambda errBead: WorkThenVolume(errBead),
                                  lambda l, e: dumpFunc(l, e)))
    knownErrors.append(BBErrorKey(FAIL_EXIF_ID, FAIL_EXIF_STR,
                                  lambda errBead: EXIFWork(errBead),
                                  lambda l, e: dumpFunc(l, e)))
    knownErrors.append(BBErrorKey(FAIL_TIKA_ID, FAIL_TIKA_STR,
                                  lambda errBead: TikaParse(errBead),
                                  lambda l, e: dumpFunc(l, e)))
    knownErrors.append(BBErrorKey(FAIL_MULTI_PAGE_TIF_ID,
                                  FAIL_MULTI_PAGE_TIF_STR,
                                  lambda errBead: TikaParse(errBead),
                                  lambda l, e: dumpErrors(l, e)))


def invertErrors(errors):
    '''
    Coalesces errors by type. Builds a dictionary whose value is a\
    list of the works which have the key's error.
    ...
    for anError in errors:
        ...
        errByType.setdefault(k.errorId, [workVolume])
